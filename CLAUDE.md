...
Calculate `overall_progress` as the weighted average of all stage progress values. Then set phase:
- `overall_progress < 0.2` → **early** (targets: R=.30, P=.30, I=.15, T=.05, E=.10, M=.10)
- `0.2 ≤ overall_progress < 0.7` → **middle** (targets: R=.05, P=.10, I=.40, T=.25, E=.10, M=.10)
- `overall_progress ≥ 0.7` → **late** (targets: R=.05, P=.05, I=.15, T=.25, E=.25, M=.25)

Update the targets in state.json if the phase changed.
...
progress_decay = (1.0 - stage.progress) * 0.2
priority_boost = 2.0 if stage is in human_priorities, else 1.0

score = (error + integral * 0.1 + value_bonus + progress_decay) * priority_boost
```
