...
def memory_write(ctx, note, heat_num, stage):
    ...
    if detected:
        ...
        if wt_root.exists() and (wt_root / ".git").exists():
            base = wt_root
    # t-568 (option-A point 2): once the per-forge layout is seeded,
    # a memory write that would land on the MAIN checkout is an ERROR,
    # not a silent write-only. Such a write leaves an uncommittable
    # dirty file (Assembly-only-to-main) attributed to whichever forge
    # id happened to resolve. Forges write from their own worktrees
    # (t-564 auto-commit); the shared rollup is GENERATED on main by
    # `smithy memory-rollup` (point 3). Unseeded rigs (pre-t-458
    # layout) keep the legacy write-only behavior.
    lands_on_main = base.resolve() == main_repo_root(root).resolve()
    # Seeded = the memory root carries the t-458 layout marker
    # (README.md, written by the migration and by init since t-568).
    # The marker — not subdir existence — keeps the policy stable: a
    # legacy fallback write creates its own subdir and must not be able
    # to arm the policy against its very next append.
    seeded = (main_repo_root(root) / "personas" / "forge" / "memory"
              / "README.md").exists()
    if lands_on_main and seeded:
        _output({"error": (
            f"memory-write would land on the MAIN checkout (forge id "
            f"{forge_id!r} resolved no worktree; cwd is not a Forge "
            f"worktree). The per-forge layout is seeded — run from your "
            f"Forge worktree so the write lands+commits there (t-564). "
            f"Shared rollups are generated via `smithy memory-rollup` "
            f"(t-568).")})
        sys.exit(2)
...
@cli.command("memory-rollup")
@click.pass_context
def memory_rollup(ctx):
    """t-568 (option-A point 3): generate the shared daily-memory rollup
    from the per-forge dirs.

    Assembly runs this on main and the result is committed there (the
    one writer allowed on main); comms CONSUMES the rollup and never
    writes memory itself. Reads every
    `personas/forge/memory/<suffix>/MEMORY_DAILY.md` and writes
    `personas/forge/memory/ROLLUP_DAILY.md` — a generated artifact, do
    not edit (distinct filename keeps patrol check #11's legacy-file
    alarm meaningful). Commit fires only when the checkout's current
    branch is `main`; elsewhere the file is written and left for the
    caller (`committed: false`).
    """
    root = ctx.obj["root"]
    mem_root = root / "personas" / "forge" / "memory"
    forges = []
    sections = []
    if mem_root.exists():
        for sub in sorted(d for d in mem_root.iterdir() if d.is_dir()):
            daily = sub / "MEMORY_DAILY.md"
            if not daily.exists():
                continue
            forges.append(sub.name)
            sections.append(
                f"## forge-{sub.name}\n\n{daily.read_text().strip()}\n")
    out = mem_root / "ROLLUP_DAILY.md"
    header = (
        "# Daily Memory — shared rollup (GENERATED)\n\n"
        "Generated by `smithy memory-rollup` (t-568) from the per-forge\n"
        "memory dirs. Do not edit: Forges write only their own subdir,\n"
        "Assembly regenerates+commits this file on main, comms consumes\n"
        "it read-only.\n\n")
    ...
    import subprocess as _sp
    try:
        br = _sp.run(["git", "rev-parse", "--abbrev-ref", "HEAD"],
                     cwd=str(root), capture_output=True, text=True,
                     timeout=10)
        if br.returncode == 0 and br.stdout.strip() == "main":
            _sp.run(["git", "add", str(out)], cwd=str(root),
                    capture_output=True, text=True, timeout=15)
            cm = _sp.run(
                ["git", "commit", "-m",
                 "[assembly] memory rollup: regenerate shared "
                 "ROLLUP_DAILY.md from per-forge dirs (t-568)",
                 "--", str(out)],
                cwd=str(root), capture_output=True, text=True, timeout=15)
            committed = cm.returncode == 0
            if not committed:
                commit_detail = (cm.stderr.strip()
                                 or cm.stdout.strip())[:200]
        else:
            commit_detail = (f"not on main (branch="
                             f"{br.stdout.strip() or '?'}) — write-only")
    except Exception as exc:
        commit_detail = str(exc)[:200]

    _output({"path": str(out), "forges": forges, "committed": committed,
             **({"commit_detail": commit_detail} if commit_detail else {})})
    _err(f"Memory rollup: {len(forges)} forge dirs -> {out.name}"
         + (" (committed)" if committed else ""))
...
def init(ctx, project_name, target, with_personas, template_name, list_templates):
    ...
    templates = {
        ...
        # t-568: README is also the seeded-layout MARKER — memory-write
        # refuses main-checkout writes when it is present.
        "personas/forge/memory/README.md": (
            "# Forge memory — per-forge layout (t-458)\n\n"
            "Each Forge writes ONLY to memory/<its-suffix>/ from its own\n"
            "worktree (auto-committed, t-564). Main-checkout writes are\n"
            "refused while this file exists (t-568); the shared rollup\n"
            "is generated via `smithy memory-rollup`.\n"),
