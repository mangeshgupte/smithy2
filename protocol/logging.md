...
- `heat`: current heat number (budget.used + 1)
...
- Increment the chosen stage's `heats` by 1
- Update stage `progress` (your estimate of how complete this stage is for the project, 0.0-1.0)
- Update stage `value_ema`: new_ema = 0.7 * old_ema + 0.3 * this_heat_value
...
- Update the **Stage Progress** table (progress %, heats, notes)
- Update **Overall progress** and **Heats used**
...
- Update the timestamp at the top: `*Updated after heat N | YYYY-MM-DD*`
...
→ ✓ done in heat 3 (wavefront model implemented)
```

Status formats:
- `→ ✓ done in heat N (description)` — implemented
