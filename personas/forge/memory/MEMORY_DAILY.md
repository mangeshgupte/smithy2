...
- [h804 editing] Heats 800-804: t-388 Cockpit polish block complete (DnD reorder with hp-in-10s + blocker guard, section split Pending/Deferred/Complete, Complete-row shape with value+heat+sha, Alt-arrow a11y + undo toast) — 12 new tests, 309/309 green. Hoisted bellows t-382 worklog helpers into smithy.worklog_agg as shared module. t-384 seeded three explicit intents across state.json (new intent field on initiatives + new intents[] list), identity.md, intents.json; research/intent-schema-gaps.md enumerates gaps (3 disconnected stores, no versioning/scoping primitive, no API, no patrol check) for t-385 to resolve. Pattern observed: when a task says 'reuse existing helpers from task X,' hoisting them into a shared module beats copying — costs one extra test file, saves drift forever.
